...
import ConfigButton from './assets/svgs/Config.svg'
...
export default function App() {
  ...
  function createTable() {
    ...
    db.transaction((tx) => {
      ...
      tx.executeSql(
        'CREATE TABLE IF NOT EXISTS todolist (id INTEGER PRIMARY KEY AUTOINCREMENT, content TEXT, subContent TEXT, done INTEGER, fixed INTEGER, categories TEXT, dday Text);'
      )
      tx.executeSql(
        'CREATE TABLE IF NOT EXISTS config (id INTEGER PRIMARY KEY AUTOINCREMENT, darktheme INTEGER DEFAULT 0, maincolor DEFAULT "cornflowerblue", LockScrenn INTEGER DEFAULT 0, font TEXT);'
      )
    })
  }
  function insertData(content, subContent, done, fixed, categories, dday = '') {
    db.transaction((tx) => {
      tx.executeSql(
        'INSERT INTO todolist(content, subContent, done, fixed, categories, dday) values (?,?,?,?,?,?)',
        [content, subContent, done, fixed, categories, dday]
      )
    })
  ...
  function paintList(category) {
    ...
    const queryString =
      category === 'ALL'
        ? 'SELECT * FROM todolist ORDER BY fixed DESC'
        : `SELECT * FROM todolist  WHERE categories LIKE '%${category}%'`
    db.transaction((tx) => {
  ...
  function updateContent(id, content, subContent, categories, dday = '') {
    db.transaction((tx) => {
      tx.executeSql(
        'update todolist set content = ?, subContent = ?, categories = ?, dday = ?  WHERE id == ?',
        [content, subContent, categories, dday, id]
      )
    })
    paintList(currentCategory)
  }
  function getCategoriesList() {
    db.transaction((tx) => {
      tx.executeSql(
        'SELECT DISTINCT categories FROM todolist',
        [],
        (_, { rows }) => {
  ...
  function updateDarkTheme() {
    db.transaction((tx) => {
      tx.executeSql('update config set darktheme = ? WHERE id == ?', [
        !darkTheme,
        1,
      ])
    })
    ...
    <ThemeProvider theme={theme}>
      <MainContainer>
        ...
        <Header>
          ...
          <TouchableOpacity
            onPress={() => {
              onChangeShowConfig(true)
            }}
          >
            {/* 캐시를 제거하고 svrrc파일을 생성후 실행해야 fill이 적용이 된다. */}
            <ConfigButton width={32} height={32} fill={theme.grey500} />
          </TouchableOpacity>
        </Header>
        <FlatList
          data={todoList}
          renderItem={({ item }) => {
            return (
              <TodoItem
                ...
                toggle={() => {
                  changeDone(item.done, item.id)
                }}
                fixedToggle={() => {
                  changeFixed(item.fixed, item.id)
                }}
                editPress={() => {
        ...
        <TaskModal
          ...
          mode={modeModal}
          hide={() => {
            onChangeshowModal(false), onchangeModeModal(true)
          }}
          ...
          edit={(content, subContent, categories, dday) => {
            ...
            console.log('엥 왜안들어와', categories)
          }}
        />
        <ConfigModal
          ...
          changeDarkTheme={() => {
            setDarkTheme(!darkTheme), updateDarkTheme()
          }}
          primaryColor={primaryColor}
          changePirmayColor={(color) => {
            setPrimaryColor(color), updatePrimaryColor(color)
          }}
        />

        <PlusButton activeOpacity={0.8} onPress={() => onChangeshowModal(true)}>
