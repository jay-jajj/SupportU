...
const TaskModal = ({
  ...
  const [subContent, onChangeSubContent] = React.useState(
    mode ? '' : originSubContent
  )
      ...
      <SubmitButton
        onPress={() => {
          if (content === '') {
            ...
            hide()
            mode
              ? add(content, subContent, convertedCategories, dateString)
              : edit(content, subContent, convertedCategories, dateString)
            resetState()
          }
        }}
      >
